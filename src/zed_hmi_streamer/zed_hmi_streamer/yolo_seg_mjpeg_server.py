...
class YoloSegStreamer(Node):
    def __init__(self):
        super().__init__("zed_yolo_seg_hmi")
    ...
    def cb(self, msg: Image):
        ...
        # Draw overlays
        out = results[0].plot()  # returns BGR numpy image with masks/boxes drawn
        ...
        if t1 - self.t0 > 2.0:
            ...
            self.get_logger().info(f"YOLO overlay FPS: {fps:.1f}")
            self.frames = 0
            self.t0 = t1
...
def root():
    return """
    <html>
      <head><title>ZED + YOLO Seg</title></head>
