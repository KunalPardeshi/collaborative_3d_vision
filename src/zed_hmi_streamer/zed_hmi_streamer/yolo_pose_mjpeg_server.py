...
class YoloPoseStreamer(Node):
    def __init__(self):
        super().__init__("zed_yolo_pose_hmi")
    ...
    def cb(self, msg: Image):
        ...
        out = results[0].plot()  # skeleton overlay
        ...
        if t1 - self.t0 > 2.0:
            ...
            self.get_logger().info(f"YOLO pose overlay FPS: {fps:.1f}")
            self.frames = 0
            self.t0 = t1
...
def root():
    return """
    <html>
      <head><title>ZED + YOLO Pose</title></head>
